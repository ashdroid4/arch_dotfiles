...
#-------------------------------CONSTANTS----------------------------------#
pacman_dependencies = (
"hyprland wayland dunst nwg-look wf-recorder wlsunset "
...
green = "\033[0;32m"; red = "\033[0;31m"; blue = "\033[0;36m"
#--------------------------------------------------------------------------#
...
3. It will write the needed config for Hyprland.""")
            
# This function will determine the input as True or False or YesForAll.
def yon(arg:str, simple=True) -> boolean:
    response = input(arg)
...
def run(arg:str):
      stdout, stderr = run(arg, shell=shell, stderr)
      stderr = stderr.decode()
      if not stderr: return 
      
      echo(f"\n\n{red}While doing the process '{arg}', the following error occured:\n{stderr}")
      proceed = yon("\nWell, I can't possibly know what error you enountered. If you don't want to proceed, the script will abort.\n"
...
if "chaotic-aur" not in pacman_conf:
      echo(f"{blue}Now, I have to install all the dependencies. To do this, I have to add another repo called Chaotic-Aur.\n"
           "I am very lazy so you have to say yes or the installation will abort.")

      ChaoticAUR = yon("Do you want to add Chaotic-Aur? "
                       "Yes or No: ")
...
print("(you can edit the dependencies as you want in the arch_dotfiles/linuxmobile_hyprland-dots)")

run(f"pacman -S --needed {pacman_dependencies}")

# Let's configure the dotfiles

run("rsync -avxHAXP --exclude '.git*' .* ~/")
