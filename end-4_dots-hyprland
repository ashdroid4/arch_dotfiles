...
#---------------------------------IMPORTS-------------------------------------#
from __init__ import echo, run, yon, chaoticAUR, checkUser, dotPath
#-----------------------------------------------------------------------------#
...
home, username = checkUser(warning=True)
...
run(f'sudo usermod -aG video,input {username}', possible_warning=temp); del temp
...
run(f"rsync -av --delete --exclude '/custom' --exclude '/hyprland.conf' {downloadPath}/.config/hypr/ {home}/.config/hypr/")
...
if hyprlandConfPath.exists():
    ...
    if skip: 
        run(f"cp {downloadPath}/.config/hypr/hyprland.conf {hyprlandConfPath}")
    else:
...
for path in (downloadPath.joinpath('.config')).iterdir():
    if path not in excludedPaths:
        if path.is_dir(): 
            run(f"rsync -av --delete {path} {home}/.config/{path.name}")
        if path.is_file(): 
            run(f"rsync -av {path} {home}/.config/{path.name}")
...
{magenta}See the 'Import Manually' folder and grab anything you need.
...
Press {green}Ctrl+Super+/{magenta} for a list of keybinds.{nocolor}"""
